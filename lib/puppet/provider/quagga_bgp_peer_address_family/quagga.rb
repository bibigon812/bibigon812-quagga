Puppet::Type.type(:quagga_bgp_peer_address_family).provide :quagga do
  ...
  @resource_map = {
    ...
    allow_as_in: {
      default: :absent,
      regexp: %r{\A\sneighbor\s(\S+)\sallowas-in\s(\d+)\Z},
      template: 'neighbor <%= name %> allowas-in<% unless value.nil? %> <%= value %><% end %>',
      type: :fixnum,
    },
    default_originate: {
      default: :false,
      regexp: %r{\A\sneighbor\s(\S+)\sdefault-originate\Z},
      template: 'neighbor <%= name %> default-originate',
      type: :boolean,
    },
    next_hop_self: {
      default: :false,
      regexp: %r{\A\sneighbor\s(\S+)\snext-hop-self\Z},
      template: 'neighbor <%= name %> next-hop-self',
      type: :boolean,
    },
    prefix_list_in: {
      default: :absent,
      regexp: %r{\A\sneighbor\s(\S+)\sprefix-list\s(\S+)\sin\Z},
      template: 'neighbor <%= name %> prefix-list <%= value %> in',
      type: :string,
    },
    prefix_list_out: {
      default: :absent,
      regexp: %r{\A\sneighbor\s(\S+)\sprefix-list\s(\S+)\sout\Z},
      template: 'neighbor <%= name %> prefix-list <%= value %> out',
      type: :string,
    },
    route_map_export: {
      default: :absent,
      regexp: %r{\A\sneighbor\s(\S+)\sroute-map\s(\S+)\sexport\Z},
      template: 'neighbor <%= name %> route-map <%= value %> export',
      type: :string,
    },
    route_map_import: {
      default: :absent,
      regexp: %r{\A\sneighbor\s(\S+)\sroute-map\s(\S+)\simport\Z},
      template: 'neighbor <%= name %> route-map <%= value %> import',
      type: :string,
    },
    route_map_in: {
      default: :absent,
      regexp: %r{\A\sneighbor\s(\S+)\sroute-map\s(\S+)\sin\Z},
      template: 'neighbor <%= name %> route-map <%= value %> in',
      type: :string,
    },
    route_map_out: {
      default: :absent,
      regexp: %r{\A\sneighbor\s(\S+)\sroute-map\s(\S+)\sout\Z},
      template: 'neighbor <%= name %> route-map <%= value %> out',
      type: :string,
    },
    route_reflector_client: {
      default: :false,
      regexp: %r{\A\sneighbor\s(\S+)\sroute-reflector-client\Z},
      template: 'neighbor <%= name %> route-reflector-client',
      type: :boolean,
    },
    route_server_client: {
      default: :false,
      regexp: %r{\A\sneighbor\s(\S+)\sroute-server-client\Z},
